...
- `vis`: Used model of viscosity. `HP` is a model taken from the HP flow calculator with a linear temperature dependency of the viscosity. `Blumberg` is an emperical formula according to the book [`[6]`](https://GasChromatographyToolbox.github.io/GasChromatographySimulator.jl/dev/references/#References)
- `control`: Control of the "Flow" or of the "Pressure" (at column inlet) during the program

For guidance on choosing an ODE solver, see [ODE Solvers](@ref).
